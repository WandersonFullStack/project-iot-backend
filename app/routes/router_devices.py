...
async def register_device(body: DeviceIn, db: DB, _: CurrentUser):
    """
...
async def list_devices(
    ...
    return await dc.list_device(
        db, active_only=active_only, limit=pag.limit, offset=pag.offset
    )
...
async def search_device(device_id: str, db: DB, _: CurrentUser):
...
async def update_device(device_id: str, body: DeviceUpdate, db: DB, _: CurrentUser):
    """
...
async def renew_api_key(device_id: str, db: DB, _: CurrentUser):
    """
    ...
    ok = await dc.renew_api_key(db, device_id, new_hash)
    if not ok:
        raise HTTPException(
            status_code=404,
            detail="Device not found."
...
async def messages_device(device_id: str, db: DB, pag: Pag, _: CurrentUser):
    """
